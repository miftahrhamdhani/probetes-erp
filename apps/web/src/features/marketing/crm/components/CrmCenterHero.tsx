import { CheckCircle2, MessageCircle, Target, Users } from "lucide-react";
import type { CrmCenterHeroStat } from "../types/crmTypes";

interface CrmCenterHeroProps {
  stats: CrmCenterHeroStat[];
}

/** Hero CRM Control Center — pill "CRM MODULE", judul besar (merah + navy), 3 fitur singkat,
 * dan visual kanan: lingkaran konsentris + ikon customer merah + kartu stat melayang.
 * Mengikuti gaya hero Sales & Order Center supaya konsisten satu modul Marketing. */
export function CrmCenterHero({ stats }: CrmCenterHeroProps) {
  return (
      ...
      <div className="relative z-10 flex flex-col gap-8 p-8 md:p-12 lg:flex-row lg:items-center lg:justify-between lg:gap-10">
        <div className="flex max-w-xl flex-col items-start">
          ...
          <h1 className="text-4xl font-extrabold tracking-[-0.03em] text-slate-900 sm:text-5xl">
            <span className="text-brand-red">CRM</span> Control Center
          </h1>
          <p className="mt-4 text-base font-medium leading-relaxed text-slate-600 sm:text-lg">
            Kelola import data, closingan, iklan CRM, dan analitik customer dalam satu sistem terintegrasi.
          </p>

          <div className="mt-8 flex flex-col gap-5 sm:flex-row sm:gap-6">
            <div className="flex items-start gap-3">
              <Users className="mt-0.5 size-6 shrink-0 text-brand-red" strokeWidth={2} />
              <div>
                <h4 className="font-bold text-slate-900">Fokus Customer</h4>
                <p className="mt-0.5 text-sm font-medium leading-tight text-slate-500">Siapa follow-up, masuk grup, dan repeat.</p>
              </div>
            </div>
            <div className="flex items-start gap-3">
              <Target className="mt-0.5 size-6 shrink-0 text-brand-red" strokeWidth={2} />
              <div>
                <h4 className="font-bold text-slate-900">Cluster Prioritas</h4>
                <p className="mt-0.5 text-sm font-medium leading-tight text-slate-500">Segmentasi custom untuk tindakan CRM.</p>
              </div>
            </div>
            <div className="flex items-start gap-3">
              <MessageCircle className="mt-0.5 size-6 shrink-0 text-brand-red" strokeWidth={2} />
              <div>
                <h4 className="font-bold text-slate-900">Tindak Lanjut Cepat</h4>
                <p className="mt-0.5 text-sm font-medium leading-tight text-slate-500">Dorong customer ke pembelian berikutnya.</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="relative hidden flex-1 items-center justify-center lg:flex">
          <div className="relative flex h-[220px] w-[220px] items-center justify-center">
            ...
            <div className="flex size-[76px] items-center justify-center rounded-2xl bg-brand-red shadow-lg shadow-brand-red/30">
              <Users className="size-9 text-white" strokeWidth={1.8} />
            </div>
          ...
          <div className="absolute -right-6 top-10 flex w-[150px] flex-col gap-1 rounded-2xl border border-slate-100 bg-white p-3.5 shadow-lg shadow-slate-200/60">
            <p className="text-[11px] font-semibold text-slate-500">{stats[1]?.label}</p>
            <p className="text-lg font-black text-slate-900">{stats[1]?.value}</p>
          </div>

          <div className="absolute -left-12 bottom-2 flex w-[164px] flex-col gap-1 rounded-2xl border border-slate-100 bg-white p-3.5 shadow-lg shadow-slate-200/60">
            <p className="inline-flex items-center gap-1.5 text-[11px] font-semibold text-slate-500">
              <CheckCircle2 className="size-3.5 text-emerald-500" />{stats[2]?.label}
            </p>
