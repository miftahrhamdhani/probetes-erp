      ...
      <div className="relative z-10 flex flex-col gap-8 p-8 md:p-12 lg:flex-row lg:items-center lg:justify-between lg:gap-12">
        <div className="flex max-w-xl flex-col items-start">
          ...
          <h1 className="text-4xl font-extrabold tracking-[-0.03em] text-slate-900 sm:text-5xl">
            <span className="text-brand-red">Marketing</span> Center
          </h1>
          <p className="mt-4 text-base font-medium leading-relaxed text-slate-600 sm:text-lg">
            Import data channel, pantau laporan penjualan, analisis iklan &amp; ROAS, dan kelola CRM dalam satu tempat.
          </p>
          <div className="mt-8 flex flex-col gap-5 sm:flex-row sm:gap-6">
            <div className="flex items-start gap-3">
              <Upload className="mt-0.5 size-6 shrink-0 text-brand-red" strokeWidth={2} />
              <div>
                <h4 className="font-bold text-slate-900">Import Channel</h4>
                <p className="mt-0.5 text-sm font-medium leading-tight text-slate-500">TikTok, Shopee, Meta, Skalev</p>
              </div>
            </div>
            <div className="flex items-start gap-3">
              <TrendingUp className="mt-0.5 size-6 shrink-0 text-brand-red" strokeWidth={2} />
              <div>
                <h4 className="font-bold text-slate-900">Laporan Penjualan</h4>
                <p className="mt-0.5 text-sm font-medium leading-tight text-slate-500">Omzet, produk, tren harian</p>
              </div>
            </div>
            <div className="flex items-start gap-3">
              <Users className="mt-0.5 size-6 shrink-0 text-brand-red" strokeWidth={2} />
              <div>
                <h4 className="font-bold text-slate-900">CRM &amp; RFM</h4>
                <p className="mt-0.5 text-sm font-medium leading-tight text-slate-500">Segmen pelanggan &amp; cohort</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="relative hidden flex-1 items-center justify-center lg:flex">
          <div className="relative flex h-[220px] w-[220px] items-center justify-center">
            ...
            {/* Orbit dots */}
            <div className="absolute right-5 top-5 size-3 rounded-full bg-[#2563EB]" title="TikTok" />
            <div className="absolute bottom-5 right-10 size-3 rounded-full bg-[#F59E0B]" title="Shopee" />
            <div className="absolute left-6 top-8 size-3 rounded-full bg-brand-red" title="Meta" />
            <div className="absolute bottom-8 left-10 size-3 rounded-full bg-[#7C3AED]" title="Skalev" />
          </div>
        </div>
      </div>
