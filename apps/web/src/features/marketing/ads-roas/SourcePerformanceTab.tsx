...
export function SourcePerformanceTab({ summary, channels, divisions, trend, orders, loading, error, title, onChannel, onDivision, onProduct, onReset }: {
  ...
  return <>
    ...
    <section className="grid grid-cols-1 gap-4 xl:grid-cols-2"><Card title="Performa Channel"><SourceTable rows={channels} label="Channel" name={(row) => `${row.channel_name} · ${row.platform}`} onClick={onChannel} /></Card><Card title="Performa Divisi"><SourceTable rows={divisions} label="Divisi" name={(row) => row.divisi} onClick={onDivision} /></Card></section>
