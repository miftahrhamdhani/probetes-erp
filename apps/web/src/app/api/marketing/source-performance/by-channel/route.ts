  ...
  try {
    ...
    const rows = await query<Record<string, unknown>>(`
      WITH valid_orders AS (
        SELECT o.order_id, o.customer_id, o.channel_id, o.total_amount FROM orders.orders o WHERE ${where}
      ), item_totals AS (
      ...
      SELECT COALESCE(vo.channel_id, 'BELUM_TERCATAT') AS channel_id,
        COALESCE(NULLIF(ch.channel_final_name, ''), 'Belum Tercatat') AS channel_name,
        COALESCE(NULLIF(ch.platform, ''), 'Belum Tercatat') AS platform,
      ...
      LEFT JOIN master.channels ch ON ch.channel_id = vo.channel_id
      LEFT JOIN item_totals it ON it.order_id = vo.order_id
      GROUP BY vo.channel_id, ch.channel_final_name, ch.platform
      ORDER BY SUM(vo.total_amount) DESC, channel_name
    `, values);
    return NextResponse.json({ success: true, data: rows, availability: SOURCE_AVAILABILITY });
  } catch (err) {
    const message = err instanceof Error ? err.message : "Kesalahan tidak diketahui.";
    console.error("GET /api/marketing/source-performance/by-channel gagal:", err);
