...
// 1. Nama produk perlu dirapikan (status review dari migrasi).
...
// 2. Produk belum berkategori (untuk RFM/cluster).
...
// 3. Pelanggan perlu dicek — HANYA nama bermasalah (bukan yang wajar tanpa HP/alamat).
const PELANGGAN_SQL = `
...
// 4. No HP tidak normal — nomor ADA tapi format salah.
const HP_SQL = `
  WITH phones AS (
    SELECT
      customer_id,
...
// 5. Kota belum terbaca — punya ALAMAT tapi kota kosong.
const KOTA_SQL = `
  SELECT
    customer_id AS id,
    COALESCE(NULLIF(name, ''), '-') AS name,
...
export async function GET(req: NextRequest) {
  ...
  try {
    if (section === "summary") return NextResponse.json(await summary());
    if (!(section in sectionSql)) {
      return NextResponse.json({ error: "Jenis kualitas data tidak dikenal." }, { status: 400 });
    }
    if (section === "kota") {
    ...
    return NextResponse.json(await query(sectionSql[section as SectionId]));
  } catch (err) {
    const message = err instanceof Error ? err.message : "Kesalahan tidak diketahui.";
    console.error("GET /api/data-quality gagal:", err);
    return NextResponse.json({ error: `Gagal memuat kualitas data: ${message}` }, { status: 500 });
  }
}
