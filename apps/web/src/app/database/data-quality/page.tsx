...
import type { KpiItem } from "@/features/database/overview/types/databaseOverview.types";
          ...
          {rows.map((row, i) => (
            <tr key={`${row.id}-${i}`} onClick={() => onSelect(row)} className="cursor-pointer transition hover:bg-slate-50">
              {columns.map((c) => (
                <td
                  key={c.key}
                  className={`py-3 pr-4 ${c.align === "right" ? "text-right font-semibold text-slate-950" : "font-medium text-slate-600"} ${c.key === "issue" ? "" : ""}`}
                >
                  {c.key === "issue" ? (
...
export default function DataQualityPage() {
  ...
  const kpiItems = [
    { label: "Produk Perlu Dirapikan", value: (summary["produk-nama"] + summary["produk-kategori"]).toLocaleString("id-ID"), detail: "Nama + kategori", tone: "amber" },
    { label: "Pelanggan Perlu Dicek", value: summary.pelanggan.toLocaleString("id-ID"), detail: "Nama bermasalah", tone: "amber" },
    { label: "No HP Tidak Normal", value: summary.hp.toLocaleString("id-ID"), detail: "Format salah", tone: "amber" },
    { label: "Kota Belum Terbaca", value: summary.kota.toLocaleString("id-ID"), detail: "Dari alamat", tone: "amber" },
  ] satisfies KpiItem[];
